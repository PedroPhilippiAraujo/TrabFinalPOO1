from classes import Usuario, Dupla_nac, Nascimento, Endereco

#Cadastro do usuário no sistema

def cadastrar():  

    x=int(input('\nQuantidade de pessoas para cadastro: '))
    while not (1 <= x):
        print('\nQuantidade inválida. Por favor, digite novamente.')
        x=int(input('Quantidade de pessoas para cadastro: '))
    
    for i in range(1,x+1):
        nome=str(input('\nNome da {}ª pessoa: '.format(i)).upper())
        while (nome == '' or nome == ' ' or nome == '  ' or nome == '   '):
            print("Erro! Dado obrigatório.")
            nome=str(input('Nome da {}ª pessoa: '.format(i)).upper())

        dia = int(input('Dia de nascimento da {}ª pessoa: '.format(i)))
        while (dia <= 0):
            print("Erro! Dado obrigatório.")
            dia = int(input('Dia de nascimento da {}ª pessoa: '.format(i)))

        mes = int(input('Mês de nascimento da {}ª pessoa: '.format(i)))
        while (mes <= 0):
            print("Erro! Dado obrigatório.")
            mes = int(input('Mês de nascimento da {}ª pessoa: '.format(i)))
        ...
        sexo = str(input('Como a {}ª pessoa define seu gênero?\n 1) Masculino\n 2) Feminino\n 3) Outros\nResposta: '.format(i)))
        ...
        if (sexo == '1'):
            ...
            sexo=str(input('Especifique o gênero da {}ª pessoa: '.format(i)).upper())

        nac=str(input('Nacionalidade da {}ª pessoa: '.format(i)).upper())
        while (nac == '' or nac == ' ' or nac == '  ' or nac == '   '):
            print("Erro! Dado obrigatório.")
            nac =str(input('Nacionalidade da {}ª pessoa: '.format(i)).upper())
        
        perg=str(input('A {}ª pessoa possui dupla nacionalidade?\nResposta: '.format(i)).upper())
        while not (perg == 'SIM' or perg == 'NÃO' or perg == 'NAO' or perg == 'S' or perg == 'N'):
            print('\nDigite "Sim" ou "Não".')
            perg=str(input('A {}ª pessoa possui dupla nacionalidade?\nResposta: '.format(i)).upper())
        
        if(perg == 'SIM' or  perg == 'S'):
            dupla=str(input('Digite a segunda nacionalidade da {}ª pessoa: '.format(i)).upper())
        ...
        municipio=str(input('Município de moradia da {}ª pessoa: '.format(i)).upper())
        while (municipio == '' or municipio == ' ' or municipio == '  ' or municipio == '   '):
            print("Erro! Dado obrigatório.")
            municipio =str(input('Município de moradia da {}ª pessoa: '.format(i)).upper())
            
        estado=str(input('Estado de moradia da {}ª pessoa: '.format(i)).upper())
        while (estado == '' or estado == ' ' or estado == '  ' or estado == '   '):
            print("Erro! Dado obrigatório.")
            estado =str(input('Estado de moradia da {}ª pessoa: '.format(i)).upper())
        
        
        endereco = Endereco(municipio,estado)
        
        
        nomemae=str(input('Nome da mãe da {}ª pessoa: '.format(i)).upper())
        if (nomemae == '' or nomemae == ' ' or nomemae == '  ' or nomemae == '   '):
            nomemae='-1'
            
        nomepai=str(input('Nome do pai da {}ª pessoa: '.format(i)).upper())
        if (nomepai == '' or nomepai == ' ' or nomepai == '  ' or nomepai == '   '):
            nomepai='-1'
        
        cpf=str(input('CPF da {}ª pessoa: '.format(i)))
        if (cpf == '' or cpf == ' ' or cpf == '  ' or cpf == '   '):
            cpf='-1'
        
        rg=str(input('RG da {}ª pessoa: '.format(i)))
        while (rg == '' or rg == ' ' or rg == '  ' or rg == '   '):
            print("Erro! Dado obrigatório.")
            rg=str(input('RG da {}ª pessoa: '.format(i)))
        
        if(perg == 'SIM' or  perg == 'S'):
            user = Dupla_nac(nome,idade,maioridade,sexo,nac,endereco,nomemae,nomepai,cpf,rg,dupla)
        else:
            user = Usuario(nome,idade,maioridade,sexo,nac,endereco,nomemae,nomepai,cpf,rg)
...
#lista de RGs cadastrados
def lista():
    
    lista = {}
    
    for user in cadastro:
        lista[user.get_rg()] = [user.get_nome()]
...
while True:
    ...
    if (pergunta == '1'):
        ...
        if cadastrofeito == False:
            ...
            rg=str(input('\nDigite o RG de quem você quer pesquisar: '))
            found = False
            for user in cadastro:
                if (rg == user.get_rg()):
                    found = True
        ...
        if cadastrofeito == False:
            ...
            rg=str(input("\nDigite o RG da pessoa cujo cadastro você deseja deletar: "))
            found = False
